    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
        ...
        <form action={async (fd: FormData) => {
          ...
          await saveArticulo(null, titulo, slug, fd.get('contenido') as string || '', fd.get('publicado') === 'on')
          ...
          <div>
            ...
            <textarea name="contenido" rows={6} placeholder="Escribe el artículo aquí..." style={{ width: '100%', border: '1px solid #e2e8f0', borderRadius: 8, padding: '8px 12px', fontSize: 13, outline: 'none', resize: 'vertical', fontFamily: 'inherit', boxSizing: 'border-box' }} />
          </div>
      ...
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
        ...
        {articulos.length === 0 ? (
        ...
        ) : articulos.map((a: { id: number; titulo: string; slug: string; publicado: boolean; contenido: string }, i: number) => (
          <div key={a.id} style={{ padding: '14px 20px', borderBottom: i < articulos.length - 1 ? '1px solid #f5f5f5' : undefined }}>
            ...
            <form action={async (fd: FormData) => {
              'use server'
              await saveArticulo(a.id, fd.get('titulo') as string, fd.get('slug') as string, fd.get('contenido') as string, fd.get('publicado') === 'on')
              ...
              <textarea name="contenido" defaultValue={a.contenido} rows={3} style={{ border: '1px solid #e2e8f0', borderRadius: 8, padding: '7px 10px', fontSize: 13, outline: 'none', resize: 'vertical', fontFamily: 'inherit' }} />
